...
public class SearchService {
    ...
    public PageResult<GoodsDTO> goodsPageQuery(SearchRequest request) {
        ...
        sortMethod(request, nativeSearchQueryBuilder);
        //添加查询条件
     ...
     * 排序方法
     * @param request 排序条件
     * @param nativeSearchQueryBuilder spring-data封装elasticsearch的复杂条件查询对象
     */
    private void sortMethod(SearchRequest request, NativeSearchQueryBuilder nativeSearchQueryBuilder) {
        Integer sortNum = request.getSortNum();
        FieldSortBuilder fsb = null;
        if (sortNum != 0) {
            switch (sortNum){
                case 1:
                    fsb = SortBuilders.fieldSort("createTime").order(SortOrder.ASC);
                    break;
                case 2:
                    fsb = SortBuilders.fieldSort("createTime").order(SortOrder.ASC);
                    break;
                case 3:
                    ...
                    break;
                case 4:
                    fsb = SortBuilders.fieldSort("price").order(SortOrder.ASC);
                    break;
                default :
                    throw new LyException(505, "排序字段数据有误！！！");
            }
            nativeSearchQueryBuilder.withSort(fsb);
        }
    }
